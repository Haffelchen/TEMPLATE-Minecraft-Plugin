 ...
 * @lastEdit v1.1
 */
public class UfTmpClearSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_UF_TMP_CLEAR.getPath());
	}
			...
			throws CommandException, TranslationNotFoundException
	{
		if(UserFormatsArg.getUFChanges().containsKey(sender))
		{
			...
			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_UF_TMP_CLEARED.getPath(), args));
			return true;

		} else
		{
			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_UF_TMP_NOTHING_TO_CLEAR.getPath(), args));
			return false;
		}
	}
	...
	{
		loadPermission(PermissionNames.ARG_UF_TMP_CLEAR);
	}
