 ...
 * @lastEdit v1.1
 */
public class UfTmpListSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_UF_TMP_LIST.getPath());
	}
			...
			throws CommandException, TranslationNotFoundException
	{
		...
		{
			for(String edited : UserFormatsArg.getUFChanges().get(sender).keySet())
			{
				if(UserFormatsArg.getUFChanges().get(sender).get(edited) instanceof ArrayList<?>)
				{
					StringBuilder list = new StringBuilder();
					int index = 0;

					for(String listValue : ((ArrayList<String>) UserFormatsArg.getUFChanges().get(sender).get(edited)))
					{
						String[] extraArgs =
						{ Integer.toString(index), listValue };

						list.append(getTranslatedString(sender, TranslationPaths.ARG_UF_TMP_LIST_ENTRY_LIST.getPath(), args, extraArgs));
						index++;
					}
					...
					changed.append(getTranslatedString(sender, TranslationPaths.ARG_UF_TMP_LIST_ENTRY.getPath(), args, extraArgs));

				} else
				{
					String[] extraArgs =
					{ edited, (String) UserFormatsArg.getUFChanges().get(sender).get(edited) };
		...
		String emptyList = getTranslatedString(sender, TranslationPaths.ARG_UF_TMP_LIST_EMPTY.getPath(), args);
		String[] extraArgs =
		{ changed.toString().isEmpty() ? emptyList : changed.toString().replaceFirst(", ", "") };

		sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_UF_TMP_CHANGED_VALUES.getPath(), args, extraArgs));
		return true;
	}
	...
	{
		loadPermission(PermissionNames.ARG_UF_TMP_LIST);
	}
