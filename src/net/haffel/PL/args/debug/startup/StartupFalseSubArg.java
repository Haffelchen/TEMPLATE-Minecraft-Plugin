 ...
 * @lastEdit v1.1
 */
public class StartupFalseSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_FALSE.getPath());
	}
	...
	{
		TEMPLATE.getInstance().getConfig().set(ConfigSettings.DEBUG_STARTUP.getPath(), false);
		TEMPLATE.getInstance().saveConfig();
		
		Config.getConfigSettings().put(ConfigSettings.DEBUG_STARTUP, false);
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_STARTUP_DISABLED.getPath(), args));
