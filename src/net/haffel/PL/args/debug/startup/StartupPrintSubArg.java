 ...
 * @lastEdit v1.1
 */
public class StartupPrintSubArg extends ArgumentBase
{
	/**
	...
	public String getArgumentDescription(CommandSender sender)
			throws TranslationNotFoundException
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_PRINT.getPath());
	}
			...
			LanguageFileNotFoundException, IOException, PluginNotRegisteredException
	{
		if(args.length > 3)
		{
			try
			{
				...
				String exception = Debug.printStartup(Integer.parseInt(args[3]), toConsole);
				...
				if(!exception.isEmpty())
				{
					...
					for(int i = 0; i < printed.length; i++)
					{
						if(i % 2 == 0)
						{
							String[] extraArgs =
							{ printed[i] };
							sb.append(TEMPLATE.getTranslatedString(null, TranslationPaths.ARG_DEBUG_STARTUP_PRINT_LINE1.getPath(), args, extraArgs));

						} else
						{
							String[] extraArgs =
							{ printed[i] };
							sb.append(TEMPLATE.getTranslatedString(null, TranslationPaths.ARG_DEBUG_STARTUP_PRINT_LINE2.getPath(), args, extraArgs));
						}
					}
					...
					sender.sendMessage(
							TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_STARTUP_PRINT.getPath(), args, extraArgs));
				...
				{
					sender.sendMessage(
							TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_STARTUP_PRINT_INVALID.getPath(), args));
	...
	{
		loadPermission(PermissionNames.ARG_DEBUG_STARTUP_PRINT);
	}
	...
	{
		ExtendedSubSyntax index = new ExtendedSubSyntax("Index", TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_PRINT_INDEX, false, false);
		ExtendedSubSyntax toConsole = new ExtendedSubSyntax("true", TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_PRINT_TO_CONSOLE, true, false);
