 ...
 * @lastEdit v1.1
 */
public class StartupGetSubArg extends ArgumentBase
{
	/**
	...
	public String getArgumentDescription(CommandSender sender)
			throws TranslationNotFoundException
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_GET.getPath());
	}
			...
			LanguageFileNotFoundException, IOException, PluginNotRegisteredException
	{
		if(args.length > 3)
		{
			try
			{
				String exception = Debug.getStartupByIndex(Integer.parseInt(args[3]));
				...
				if(!exception.isEmpty())
				{
					...
					sender.sendMessage(
							TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_STARTUP_GET.getPath(), args, extraArgs));

					return true;
				} else
				{
					sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_STARTUP_GET_INVALID.getPath(), args));
	...
	{
		loadPermission(PermissionNames.ARG_DEBUG_STARTUP_GET);
	}
	...
	{
		ExtendedSubSyntax index = new ExtendedSubSyntax("Index", TranslationPaths.DESCRIPTION_ARG_DEBUG_STARTUP_GET_INDEX, false, false);
