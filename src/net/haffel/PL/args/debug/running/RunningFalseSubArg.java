 ...
 * @lastEdit v1.1
 */
public class RunningFalseSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_RUNNING_FALSE.getPath());
	}
	...
	{
		TEMPLATE.getInstance().getConfig().set(ConfigSettings.DEBUG_RUNNING.getPath(), false);
		TEMPLATE.getInstance().saveConfig();
		
		Config.getConfigSettings().put(ConfigSettings.DEBUG_RUNNING, false);
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_RUNNING_DISABLED.getPath(), args));
	...
	{
		loadPermission(PermissionNames.ARG_DEBUG_RUNNING_ENABLED);
	}
}
