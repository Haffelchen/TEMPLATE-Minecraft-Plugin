 ...
 * @lastEdit v1.1
 */
public class RunningTrueSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_RUNNING_TRUE.getPath());
	}
	...
	{
		TEMPLATE.getInstance().getConfig().set(ConfigSettings.DEBUG_RUNNING.getPath(), true);
		TEMPLATE.getInstance().saveConfig();
		
		Config.getConfigSettings().put(ConfigSettings.DEBUG_RUNNING, true);
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_RUNNING_ENABLED.getPath(), args));
