 ...
 * @lastEdit v1.1
 */
public class RunningListSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_RUNNING_LIST.getPath());
	}
	...
	{
		String emptyList = TEMPLATE.getTranslatedString(TranslationPaths.ARG_DEBUG_RUNNING_LIST_EMPTY.getPath());
		String list = Debug.getListOfRunnings();
		String[] extraArgs = { list.isEmpty() ? emptyList : list };
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_RUNNING_LIST.getPath(), args, extraArgs));
	...
	{
		loadPermission(PermissionNames.ARG_DEBUG_RUNNING_LIST);
	}
}
