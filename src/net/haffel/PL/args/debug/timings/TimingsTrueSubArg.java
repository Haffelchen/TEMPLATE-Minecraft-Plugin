 ...
 * @lastEdit v1.1
 */
public class TimingsTrueSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_TIMINGS_TRUE.getPath());
	}
	...
	{
		TEMPLATE.getInstance().getConfig().set(ConfigSettings.DEBUG_TIMINGS.getPath(), true);
		TEMPLATE.getInstance().saveConfig();
		
		Config.getConfigSettings().put(ConfigSettings.DEBUG_TIMINGS, true);
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_TIMINGS_ENABLED.getPath(), args));
