 ...
 * @lastEdit v1.1
 */
public class TimingsFalseSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_DEBUG_TIMINGS_FALSE.getPath());
	}
	...
	{
		TEMPLATE.getInstance().getConfig().set(ConfigSettings.DEBUG_TIMINGS.getPath(), false);
		TEMPLATE.getInstance().saveConfig();
		
		Config.getConfigSettings().put(ConfigSettings.DEBUG_TIMINGS, false);
		
		sender.sendMessage(TEMPLATE.getTranslatedStringSplitted(sender, TranslationPaths.ARG_DEBUG_TIMINGS_DISABLED.getPath(), args));
	...
	{
		loadPermission(PermissionNames.ARG_DEBUG_TIMINGS_ENABLED);
	}
}
