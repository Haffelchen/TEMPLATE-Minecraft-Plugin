 ...
 * @lastEdit v1.1
 */
public class LangResetSubArg extends ArgumentBase
{
	/**
	...
	public String getArgumentDescription(CommandSender sender)
			throws TranslationNotFoundException
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_LANG_RESET.getPath());
	}
	...
	public boolean execute(CommandSender sender, Command cmd, String label, String[] args) throws CommandException, TranslationNotFoundException,
			IOException, LanguageFileNotFoundException, LanguageNotLoadedException
	{
		if(args.length > 2)
		{
			if(LanguageFile.isLanguageLoaded(TEMPLATE.getPluginName(), args[2]))
			{
				sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_RESETTING.getPath(), args));

				LanguageFile.resetLanguage(TEMPLATE.getPluginName(), args[2]);

				sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_RESETTET.getPath(), args));

				return true;
			} else
			{
				sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_RESET_NOT_LOADED.getPath(), args));

				return false;
			}
		} else
		{
			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_RESETTING_DEFAULT.getPath(), args));

			LanguageFile.resetLanguage(TEMPLATE.getPluginName(), Config.getConfigSettings().get(ConfigSettings.LANGUAGE) + "");

			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_RESETTET_DEFAULT.getPath(), args));
	...
	{
		loadPermission(PermissionNames.ARG_LANG_RESET);
	}
	...
	{
		ExtendedSubSyntax language = new ExtendedSubSyntax("Language", TranslationPaths.DESCRIPTION_ARG_LANG_RESET_LANGUAGE, true, false);
