 ...
 * @lastEdit v1.1
 */
public class LangTmpClearSubArg extends ArgumentBase
{
	/**
	...
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_LANG_TMP_CLEAR.getPath());
	}
			...
			throws CommandException, TranslationNotFoundException
	{
		if(LanguageArg.getLanguageChanges().containsKey(sender))
		{
			...
			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_TMP_CLEARED.getPath(), args));
			return true;

		} else
		{
			sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_TMP_NOTHING_TO_CLEAR.getPath(), args));
			return false;
		}
	}
	...
	{
		loadPermission(PermissionNames.ARG_LANG_TMP_CLEAR);
	}
