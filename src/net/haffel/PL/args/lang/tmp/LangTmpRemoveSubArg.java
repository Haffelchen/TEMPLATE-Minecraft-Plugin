 ...
 * @lastEdit v1.1
 */
public class LangTmpRemoveSubArg extends ArgumentBase
{
	/**
	...
	public String getArgumentDescription(CommandSender sender)
			throws TranslationNotFoundException
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_LANG_TMP_REMOVE.getPath());
	}
			...
			throws CommandException, TranslationNotFoundException
	{
		if(args.length > 3)
		{
			...
			{
				if(LanguageArg.getLanguageChanges().get(sender).containsKey(args[3]))
				{
					LanguageArg.getLanguageChanges().get(sender).remove(args[3]);

					sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_TMP_REMOVED_VALUE.getPath(), args));
					return true;

				} else
				{
					sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_TMP_DOES_NOT_CONTAIN.getPath(), args));
				}
			} else
			{
				sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_LANG_TMP_NOTHING_TO_REMOVE.getPath(), args));
			}
		}
	...
	{
		loadPermission(PermissionNames.ARG_LANG_TMP_REMOVE);
	}
	...
	{
		ExtendedSubSyntax key = new ExtendedSubSyntax("Key", TranslationPaths.DESCRIPTION_ARG_LANG_TMP_REMOVE_KEY, false, false);
			...
			throws TranslationNotFoundException, PluginNotRegisteredException
	{
		if(args.length == 4)
		{
			return super.getListOfStringsMatchingLastWord(args, LanguageArg.getLanguageChanges().get(sender).keySet());
		}
