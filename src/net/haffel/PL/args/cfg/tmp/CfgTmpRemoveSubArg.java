 ...
 * @lastEdit v1.1
 */
public class CfgTmpRemoveSubArg extends ArgumentBase
{
	/**
	...
	public String getArgumentDescription(CommandSender sender)
			throws TranslationNotFoundException
	{
		return TEMPLATE.getTranslatedString(sender, TranslationPaths.DESCRIPTION_ARG_CFG_TMP_REMOVE.getPath());
	}
			...
			throws CommandException, TranslationNotFoundException
	{
		if(args.length > 3)
		{
			...
			{
				if(ConfigArg.getConfigChanges().get(sender).containsKey(args[3]))
				{
					ConfigArg.getConfigChanges().get(sender).remove(args[3]);

					sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_CFG_TMP_REMOVED_VALUE.getPath(), args));
					return true;

				} else
				{
					sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_CFG_TMP_DOES_NOT_CONTAIN.getPath(), args));
				}
			} else
			{
				sender.sendMessage(getTranslatedStringSplitted(sender, TranslationPaths.ARG_CFG_TMP_NOTHING_TO_REMOVE.getPath(), args));
			}
		}
	...
	{
		loadPermission(PermissionNames.ARG_CFG_TMP_REMOVE);
	}
	...
	{
		ExtendedSubSyntax key = new ExtendedSubSyntax("Key", TranslationPaths.DESCRIPTION_ARG_CFG_TMP_REMOVE_KEY, false, false);
			...
			throws TranslationNotFoundException
	{
		if(args.length == 4)
		{
			if(ConfigArg.getConfigChanges().containsKey(sender))
			{
				return getListOfStringsMatchingLastWord(args, ConfigArg.getConfigChanges().get(sender).keySet());
			}
		}
